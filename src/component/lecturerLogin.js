...
const LecturerLogin = (props) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost:5000/api/lecturer/login", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.message); // Show error toast notification
      }
    } catch (error) {
        ...
        <div className="card">
          <div className="card-body">
            <div className="flex justify-center">
              <h2 className="text-center">Lecturer Login</h2>
            </div>
